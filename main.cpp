...
int main() {
    ...
    double L[21] = {0};    // distance between Alice and Bob
    for(int i = 0; i < 21; ++i) {
        L[i] = 100 + 45*i;
    }
                    ...
                    for(int k = 1; k < testRange; ++k) {    // level-3
                        ...
                        for(int l = 1; l < testRange; ++l) {    // level-2
                            ...
                            for (int m = 1; m < testRange; ++m) {   // level-1
                                ...
                                double C1 = tGene1*numQR*Latt/(tauPh*L[LInd]*f*transProb);
                                double rs1 = f*transProb/(tGene1*pow(10, -9));
                                double C2 = tGene2*numQR*Latt/(tauPh*L[LInd]*f*transProb);
